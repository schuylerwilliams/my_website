...
function chartDataTwo(x) {
	for (i=1; i <= 12; i++) {
		...
		if (setDataGraphTwo > 0) {
			...
			document.getElementById(dataGraphTwo[i-1]).style.background = 'rgba(256,102,102,1)';
		} else if (setDataGraphTwo < 0) {
			setDataGraphTwoMult = (setDataGraphTwo * -1) * 1.5;
			document.getElementById(dataGraphTwo[i-1]).style.background = 'rgba(102,255,178,1)';
		}
...
function chartDataOne(x) {
	for (i=1; i <= 12; i++) {
		...
		if (setDataGraphOne > 0) {
			...
			document.getElementById(dataGraphOne[i-1]).style.background = 'rgba(256,102,102,1)';
		} else if (setDataGraphOne < 0) {
			setDataGraphOneMult = (setDataGraphOne * -1) * 1.5;
			document.getElementById(dataGraphOne[i-1]).style.background = 'rgba(102,255,178,1)';
		}
...
function buttonYearMouseOver(x) {
	...
	buttonYearHolder.style.backgroundColor = "black";
